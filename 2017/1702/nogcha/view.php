...
<?php 
  ...
  if( $no ) {
    $sql = "SELECT * FROM nognews WHERE no={$no}";
    $rs = $db->query($sql);
  }
  if( $rs->rowCount() ) {
    $news = $rs->fetch();
...
<?php
  $sql = "SELECT writer FROM nognews WHERE no={$no}";
  $rs = $db->query($sql);
  $row = $rs->fetch();
  $writer = $row['writer'];
...
<?php 
  $sql = "SELECT * FROM nogcomment WHERE no={$no}";
  $rs = $db->query($sql);
...
<?php
  $sql = "SELECT * FROM nogcomment WHERE no={$no} ORDER BY usertime ASC";
  $rs = $db->query($sql);
  $coms = $rs->fetchAll();
  foreach($coms as $com) {
?>
